...
int main(int argc, char *argv[]) {
  ...
  while (ret && rclcpp::ok()) {
    ...
    // Set parameters for the LaserScan message
    ...
    // Calculate the size for ranges and intensities based on 240 degrees
    double min_angle_rad = scan.config.min_angle;
    double max_angle_rad = scan.config.min_angle + 240 * M_PI / 180.0; // Convert degrees to radians
    int size = std::ceil((max_angle_rad - min_angle_rad) / scan.config.angle_increment);

    // Resize ranges and intensities
    scan_msg->ranges.resize(size);
    scan_msg->intensities.resize(size);

    // Fill ranges and intensities based on scan.points
    for (size_t i = 0; i < scan.points.size(); i++) {
      double angle_rad = scan.points[i].angle;
      if (angle_rad >= min_angle_rad && angle_rad <= max_angle_rad) {
        int index = std::ceil((angle_rad - min_angle_rad) / scan.config.angle_increment);
